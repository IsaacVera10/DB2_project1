...
private:
    ...
    void createFile()
    {
        ...
        for (int i = 0; i < binaryStrings.size(); ++i) {
            HashIndex hashIndex;
            hashIndex.binary = stoi(binaryStrings[i], nullptr, 2);
            hashIndex.bucketAddress = (this->makeAddress(binaryStrings[i], 1) == 0) ? bucket.bucketAddress : bucket2.bucketAddress;
    ...
    Record* searchInBucket(Bucket &bucket, T key) {
        for (int i = 0; i < bucket.size; i++) {
            //cout << "Key: " << key << endl;
            //cout << "bucket.records[i].getKey(): " << bucket.records[i].getKey() << endl;
    ...
    int makeAddress(string binaryString, int depth) { //Returns position of bucket depending on its local depth
        int response = 0;
        int mask = 1;
        int binary = std::stoi(binaryString, nullptr, 2);
        ...
        int reversedResponse = 0;
        while (response) {
            reversedResponse <<= 1;
            reversedResponse |= response & 1;
            response >>= 1;
        }
...
public:
    ...
    void scanAll(){
        ...
        while(dataFile.read(reinterpret_cast<char*>(&bucket), sizeof(bucket))){
            cout << "Bucket: " << contador  << " Next Bucket: " << bucket.next_bucket;
            cout << " Binary: " << bucket.toBinaryString() << " Index: " << bucket.binary << endl;
            cout << " Bucket Address: " << bucket.bucketAddress << " Depth: " << bucket.depth << endl;
    ...
    void printIndexVector() {
        for (HashIndex index : this->indexVector) {
            cout << index.binary << " " << index.bucketAddress << endl;
        }
    }
};
